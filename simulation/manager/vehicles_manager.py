            ...
            if len(self.vehicles) < self.num_of_cars:
                for lane in lanes:
                    ...
                    for vehicle in self.vehicles:
                        if vehicle.location.y == lane - 2.5 and vehicle.location.x <= 50:
                            space_available = False
                            break
                    if space_available:
                        car_probability = random.uniform(0, 1)
                        if car_probability >= 0.25:
                            newCar = Car(Point(-simulationWorld.maxSpeed, lane - 2.5), speed=simulationWorld.maxSpeed)
                            newCar.setDesiredSpeed(simulationWorld.maxSpeed)
                            self.vehicles.append(newCar)
                            break
                        else:
                            newTruck = Truck(Point(-simulationWorld.maxSpeed, lane - 4), speed=simulationWorld.maxSpeed)
